<template>
    <DemoSettings>
        <div class="flex items-center gap-2">
            <Checkbox
                v-model="isCustomScrollbar"
                input-id="fieldset-toggleable"
                binary
            />
            <label for="fieldset-toggleable">Custom Scrollbar</label>
        </div>
    </DemoSettings>

    <DemoGroup>
        <ScrollPanel
            :dt="!isCustomScrollbar ? undefined : {
                bar: {
...
<script setup lang="ts">
    ...
    const isCustomScrollbar = ref(false);
</script>